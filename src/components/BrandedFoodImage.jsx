...
export default function BrandedFoodImage({ src, name, category, className = '', alt = '' }) {
  ...
  const getCategoryIllustration = () => {
    ...
    if (catLower.includes('burger')) {
      return (
        <svg className="w-12 h-12 text-french-gold opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 10c0-3.3 3.6-6 8-6s8 2.7 8 6v1H4v-1zM4 14h16v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM4 11h16" />
        </svg>
      );
    }
    if (catLower.includes('momo')) {
      return (
        <svg className="w-12 h-12 text-french-gold opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 4c-4.4 0-8 3.6-8 8s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8zm0 3a2 2 0 100 4 2 2 0 000-4z" />
        </svg>
      );
    }
    if (catLower.includes('fries') || catLower.includes('starter') || catLower.includes('loaded')) {
      return (
        <svg className="w-12 h-12 text-french-gold opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M19 5l-2 14H7L5 5m0 0h14M9 3v2m6-2v2" />
        </svg>
      );
    }
    if (catLower.includes('roll') || catLower.includes('sandwich')) {
      return (
        <svg className="w-12 h-12 text-french-gold opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 8h10M7 12h10M7 16h10M4 6h16a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V8a2 2 0 012-2z" />
        </svg>
      );
    }
