...
export default function OrderModeToggle({ showDetailsCard = true }) {
  ...
  const modes = [
    { id: 'dine-in', label: 'Dine-In', icon: Utensils, desc: 'Eating at cafe' },
    { id: 'takeaway', label: 'Takeaway', icon: ShoppingBag, desc: 'Pick up in 20m' },
    { id: 'delivery', label: 'Delivery', icon: Bike, desc: 'Free under 2km' },
  ];

  return (
    <div className="w-full max-w-2xl mx-auto my-4">
      {/* Question Tagline */}
      <p className="text-center font-serif text-xl sm:text-2xl text-french-dark font-extrabold mb-2 tracking-wide">
        Select Your Order Preference
      </p>

      {/* Segmented Control Pill Bar */}
      <div className="relative p-1.5 rounded-full bg-french-dark/90 border border-french-gold/40 shadow-2xl backdrop-blur-md flex items-center justify-between">
        {modes.map((mode) => {
          ...
          return (
            <button
              ...
              className={`relative flex-1 py-3 px-3 rounded-full text-xs sm:text-sm font-bold transition-all duration-300 flex items-center justify-center gap-2 z-10 ${
                isSelected
            ...
            >
              ...
              {isSelected && (
                <CheckCircle2 className="w-4 h-4 ml-0.5 text-french-dark hidden sm:inline shrink-0" />
              )}
            </button>
          );
        })}
      ...
      {showDetailsCard && (
        <div className="mt-3 p-4 rounded-2xl bg-french-card border border-french-gold/30 shadow-md backdrop-blur-sm transition-all animate-fadeIn">
          {orderMode === 'dine-in' && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
              ...
              <button
                ...
                className="text-xs font-bold text-french-gold hover:underline flex items-center gap-1.5 bg-french-dark/90 px-3.5 py-1.5 rounded-full border border-french-gold/30"
              >
                <span>Change Table Details</span>
          ...
          {orderMode === 'takeaway' && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-french-gold/15 text-french-gold flex items-center justify-center">
                  <ShoppingBag className="w-5 h-5 text-french-gold" />
                </div>
                <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-bold text-emerald-800 flex items-center gap-1.5">
                    <span>You are within 2 km — delivery is free</span>
                  </span>
