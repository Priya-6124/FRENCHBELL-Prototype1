...
import {
  ...
  Bike, CreditCard, BarChart3, Settings, LogOut, ArrowLeft, ShieldCheck, QrCode
} from 'lucide-react';
...
export default function AdminSidebar({ activeTab, setActiveTab, onBackToSite, onLogout }) {
  const menuNav = [
    ...
    { id: 'table-qr', label: 'Table QR Standees', icon: QrCode, badge: 'QR' },
              ...
              >
                ...
                {item.badge && (
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase ${
                    item.badge === 'Live' ? 'bg-red-600 text-white animate-pulse' : 'bg-french-gold/20 text-french-gold border border-french-gold/40'
                  }`}>
                    {item.badge}
                  </span>
                )}
